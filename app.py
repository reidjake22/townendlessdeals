...
from modules.conversations.manager import ConversationManager
...
def sms_reply():
    ...
    if not scheduler_started:
        ...
        welcome_message = "✨💖 OMG HIIII BIRTHDAY QUEEN!!! 💖✨\n\n" + \
        ...
        # Store the welcome message in conversation history
        conversation_manager.add_message(from_number, "assistant", welcome_message)
        
        resp.message(welcome_message)
        app.logger.info(f"Scheduler activated by {from_number}")
        return Response(str(resp), mimetype="application/xml")
    
    # Handle ongoing conversations using the conversation manager
    response = conversation_manager.process_user_message(from_number, incoming_msg)
